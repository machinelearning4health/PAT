...
def pat(fuzz_val, top_k_words, qrs, sample_index, hypotheses, premise, true_label,
        ...
        if flag == 1:
            ...
            for t in range(100):  
                ...
                for perturb_idx in range(len(nonzero_ele)):
                    ...
                    text_ls_word_emb = np.asarray([float(num) for num in embed_content[word_idx_dict[  synonyms_all[perturb_word_idx][1][0]  ]].strip().split()[1:]])
                    pos_radius = []
                    neg_radius = []
                    for _ in range(n_sample):
                        ...
                        deviate_vector = np.asarray([float(num) for num in embed_content[word_idx_dict[replacement]].strip().split()[1:]])
                        ...
                        if np.sum(get_attack_result([x_t_tmp], premise, predictor, orig_label, batch_size))>0:
                            ...
                            pos_radius.append(deviate_vector )
                            if sim_tmp > best_sim:
                                best_attack = x_t_tmp[:]
                                best_sim = sim_tmp
                            ...
                            syn_sim_set.append(0.0)
                            neg_radius.append(deviate_vector )
                        
                   
                    syn_sim_set_np = np.asarray(syn_sim_set)
                   
                    if np.max(syn_sim_set_np)==np.min(syn_sim_set_np):
                        replacement = orig_word
                        
                        x_tilde[synonyms_all[perturb_word_idx][0]] = replacement
                    else:
                        neg_center = np.mean(neg_radius, axis=0)
                        pos_center = np.mean(pos_radius, axis=0)
                        ...
                        for syn in synonyms_all[perturb_word_idx][1]:
                            ...
                            pos_center_dis = np.sum((pos_center-syn_feat)**2)
                            neg_center_dis = np.sum((neg_center-syn_feat)**2)
                            
                            if pos_center_dis< neg_center_dis:
                                predict_label_set.append(1.0)
                                
                                if pos_center_dis == 0.0:
                                    pos_dist_val = np.sum((neg_center-pos_center)**2)
                                else:
                                    pos_dist_val = np.sum((syn_feat-neg_center)**2)*abs(1-spatial.distance.cosine((neg_center-pos_center).tolist(), (syn_feat-neg_center).tolist()))**2
                                pos_dist_set.append(pos_dist_val)
                                cos_sim_set.append(pos_dist_val)
                            else:
                                predict_label_set.append(0.0)
                                cos_sim_set.append(0.0)

                           
                        if sum(predict_label_set) == 0.0:
                            replacement = orig_word
                            x_tilde[synonyms_all[perturb_word_idx][0]] = replacement
                        else:
                            
                            pos_dist_set.sort()
                            for ele in pos_dist_set:
                                perturb_syn_order = cos_sim_set.index(ele)
...
def main():
    ...
    log_file = "results_nli_hard_label/"+args.target_model+"/"+args.target_dataset+"/pat_update_log.txt"
    result_file = "results_nli_hard_label/"+args.target_model+"/"+args.target_dataset+"/pat_results_final.csv"
    ...
    for idx, premise in enumerate(data['premises']):
        ...
        if args.perturb_ratio == 0:
            ...
            new_label, num_queries, sim, random_sim = pat(args.fuzz,args.top_k_words,args.allowed_qrs,
    ...
    with open(os.path.join(args.output_dir, 'pat_adversaries.txt'), 'w') as ofile:
